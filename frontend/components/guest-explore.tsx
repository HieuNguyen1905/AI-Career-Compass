...
export function GuestExplore({ careers }: { careers: CareerPath[] }) {
  const { allMatches, assessmentReady, isLoading, error } = useGuestMatches(10);
  ...
  const libraryCareers = [
    ...allMatches.slice(5),
    ...careers.filter((career) => !matchMap.has(career.id)),
  ];
          ...
          <div className="relative flex flex-col items-start justify-between gap-5 md:flex-row md:items-center">
            <div className="flex gap-4">
              ...
              <div>
                ...
                {isLoading ? (
                  <p className="m-0 mt-2 text-sm font-semibold text-amber-700">Đang tải kết quả dùng thử...</p>
                ) : null}
                {error ? (
                  <p className="m-0 mt-2 text-sm font-semibold text-rose-700">{error}</p>
                ) : null}
              </div>
            </div>
