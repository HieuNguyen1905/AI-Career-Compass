    ...
    <main className="grid min-h-[100dvh] grid-cols-1 lg:min-h-screen lg:grid-cols-[minmax(320px,0.9fr)_minmax(380px,1.1fr)]">
      <section className="relative flex min-h-[auto] flex-col justify-between gap-4 overflow-hidden p-6 text-white sm:p-10 lg:min-h-screen lg:gap-0 lg:p-12">
        ...
        <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
          <div className="animate-aurora absolute -top-24 right-0 h-96 w-96 rounded-full bg-amber-400/20 blur-3xl" />
          <div className="animate-aurora absolute bottom-0 -left-16 h-80 w-80 rounded-full bg-teal-500/25 blur-3xl [animation-delay:-8s]" />
        </div>
        ...
        <div>
          ...
          <h1 className="mt-4 mb-2 max-w-xl font-display text-2xl font-extrabold leading-[1.15] tracking-tight sm:mt-10 sm:mb-4 sm:text-3xl lg:text-[2.8rem] lg:leading-[1.08]">
            Khởi đầu hành trình <span className="text-teal-300">nghề nghiệp</span>
          </h1>
          <p className="m-0 max-w-lg text-sm leading-relaxed text-white/70 sm:text-base lg:text-[17px]">
            Khám phá bản thân, hiểu rõ thế mạnh và nhận các gợi ý định hướng chuyên sâu được thiết kế dành riêng cho bạn.
          </p>
        </div>

        <p className="mt-2 flex items-center gap-2 text-xs text-white/50 sm:text-sm lg:mt-8">
          <Sparkles size={16} aria-hidden className="shrink-0 text-amber-300" />
          <span>Hành trình hướng nghiệp bắt đầu từ việc hiểu chính mình.</span>
        </p>
      </section>
      ...
      <section className="flex flex-1 items-center justify-center bg-white/40 p-4 py-8 backdrop-blur sm:p-8 lg:min-h-screen lg:grid lg:place-items-center">
        <div className="card w-full max-w-[440px] p-6 sm:p-8 animate-fade-up">
          ...
          <h2 className="m-0 font-display text-xl sm:text-2xl font-extrabold tracking-tight text-slate-900">Đăng ký tài khoản</h2>
          <p className="mt-1.5 sm:mt-2 mb-5 sm:mb-6 text-sm sm:text-base leading-relaxed text-slate-500">
            Bắt đầu hành trình định hướng tương lai của bạn ngay hôm nay.
          </p>
          <RegisterForm />
        </div>
      </section>
